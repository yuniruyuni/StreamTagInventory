...
test("フォーカス時にアクティブクラスが適用される", async () => {
  ...
  expect(list).toHaveClass("border-on-surface/20");
  expect(list).not.toHaveClass("border-on-surface");
  ...
  expect(list).toHaveClass("border-on-surface");
...
test("タグが上限に達するとエラーボーダーとエラーメッセージが表示される", () => {
  ...
  // エラーボーダーが適用されていること
  const fieldset = getByRole("group");
  expect(fieldset).toHaveClass("border-error");
...
test("タグが上限近く（8個以上）で警告色が表示される", () => {
  ...
  const counter = getByTestId("tag-counter");
  expect(counter.parentElement).toHaveClass("text-warning");
});
