...
describe("OidcNonceRepository upsert + get", () => {
  test("inserts and retrieves a nonce", async () => {
    ...
    expect(retrieved?.nonce).toBe(nonce.nonce);
  });

  test("returns null for non-existent nonce", async () => {
    const retrieved = await repo.get(rCtx, OidcNonce.ByValue("nonexistent"));
...
describe("OidcNonceRepository atomic consume pattern", () => {
  test("delete(ByValue AND ActiveAt(now)) is the atomic consume", async () => {
    ...
    const deleted = await repo.delete(
      wCtx,
      and(OidcNonce.ByValue("fresh-nonce"), OidcNonce.ActiveAt(now)),
    );
    expect(deleted).toBe(1);
    expect(await repo.get(rCtx, OidcNonce.ByValue("fresh-nonce"))).toBeNull();
  });
  ...
  test("ExpiredAt(now) cleanup removes only past-expired rows", async () => {
    ...
    expect(deleted).toBe(1);
    expect(await repo.get(rCtx, OidcNonce.ByValue("fresh"))).not.toBeNull();
    expect(await repo.get(rCtx, OidcNonce.ByValue("stale"))).toBeNull();
  });
});
