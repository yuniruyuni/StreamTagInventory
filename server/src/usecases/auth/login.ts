 ...
 * - 結果は `{ user, session, rawSessionToken }`。`rawSessionToken` は client が
 *   sessionStorage に保管 + `Authorization: Bearer` で送信する raw bearer token
 *   (base64url)。ADR 0006 に従い DB には `sha256(raw)` を `session.tokenHash` として
 *   保存するのみで、raw は応答後にサーバ側から消える。presentation 層は login の
 *   応答 body にこの raw を含めて返す
 */
export const login = usecase({
  ...
  write: async (ctx, { idToken, nonce: nonceString }) => {
    ...
    // 4. session を新規作成。
    //    - bearer token: ADR 0006 に従い raw を発行し、DB には sha256 だけ置く。
